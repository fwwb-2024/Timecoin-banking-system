...
<script>
import {completeRemark, getTask, getTaskDetail, noCompleteRemark} from "@/api/api";

export default {
  name: "taskCompleteReview",
  data(){
    return {
      tableData:[],
      // 显示任务详情
  ...
  methods:{
    ...
    // 审核通过
    remarkPass(){
      completeRemark(this.task.taskID).then((res)=>{
        if(res.data == '审核成功'){
          this.$message({
            message: '审核通过',
            type: 'success'
    ...
    noRemarkPass(){
      ...
      noCompleteRemark(this.task.taskID,tempRamark).then((res)=>{
        if(res.data == '批改完成'){
          this.$message({
            message: '已拒绝',
            type: 'success'
