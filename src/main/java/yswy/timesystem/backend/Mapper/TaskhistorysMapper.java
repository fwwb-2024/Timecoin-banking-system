...
public interface TaskhistorysMapper {
    ...
    @Select("select `task_history_id` as taskHistoryID,`user_id` as userID,`task_id` as taskID,`task_history_status` as taskHistoryStatus from `tasks` where `user_id` = #{userID} limit 10 offset #{offSet};")
    @Transactional
