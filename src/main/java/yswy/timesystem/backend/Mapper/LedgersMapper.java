...
public interface LedgersMapper {
    ...
    //查看账单，根据用户id查找
    @Select("select `ledger_id` as ledgerID,`user_id` as userID,`task_id` as taskID,`ledger_time_coin` as ledgerTimeCoin from `familyusers` where `user_id` = #{userId} limit 10 offset #{offSet};")
    @Transactional
